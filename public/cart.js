...
function loadedHandler() {
    ...
    if (document.getElementById("cartText")) { //Update cart on checkout page
        let cartText = document.getElementById("cartText")
        var array = JSON.parse(sessionStorage.getItem('theCart'))
        if (array.length == 0) {
            cartText.value = "Your cart is empty. <br> You can add items to the cart from the ordering page. "
        }
